---
name: ❓ Question
about: Faça uma pergunta sobre o projeto
title: '[QUESTION] '
labels: question
assignees: ''
---

## ❓ Pergunta

<!-- Faça sua pergunta de forma clara -->

## 🔍 Contexto

<!-- Forneça contexto para sua pergunta -->

**O que você está tentando fazer?**
<!-- Descreva seu objetivo -->

**O que você já tentou?**
<!-- Liste o que já foi tentado -->

## 📚 Pesquisa Prévia

<!-- Marque onde você já procurou -->
- [ ] Li o README.md
- [ ] Consultei a documentação em docs/
- [ ] Pesquisei em issues existentes
- [ ] Consultei GitHub Discussions
- [ ] Pesquisei no Google/Stack Overflow

## 💻 Ambiente (se relevante)

- OS: [e.g. Ubuntu 22.04]
- Docker Version: [e.g. 24.0.6]
- Lab Version: [e.g. v1.0.0]

## 📸 Screenshots/Logs (se relevante)

<!-- Adicione screenshots ou logs que ajudem a entender -->

## 🎯 Resultado Esperado

<!-- Como você espera que funcione? -->

## ⚠️ Nota

<!-- 
💡 Considere usar GitHub Discussions para perguntas:
https://github.com/seu-usuario/docker-lab/discussions

Issues são melhor usadas para bugs e features.
Discussions são ideais para perguntas e ajuda.
-->
